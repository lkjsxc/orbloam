...
BINARY = os.environ.get('LKJSCRIPT', str(ROOT / '.runtime/lkjscript/lkjscript'))
...
class Author:
    ...
    def fn(self, name, parameters, result, body, effects=()):
        ...
        self.emit(f'expression.block as=${name}-body\n  {body}\nexpression.end')
        self.emit(f'create.function as=${name} module=$game name={name} visibility=private result={result} effect={"task" if effects else "pure"} body=${name}-body')
    ...
    def response(self, status, body, content_type='application/json; charset=utf-8'):
        return rec(status=lit(status), body=body, headers='(list @Header ' + ' '.join(rec(name=lit(k), value=self.call('bytes-from-text', lit(v))) for k, v in [('content-type', content_type), ('cache-control', 'no-store'), ('x-content-type-options', 'nosniff'), ('referrer-policy', 'no-referrer')]) + ')')
    ...
    def bootstrap(self):
        ...
        self.emit('create.target as=$world name=world component=$component runner=http')
        self.schema()

    def schema(self):
        identity = 'orbloam-state-v1'
...
def prototype(a):
    a.structure('Counter', {'number': 'i64'})
    fallback = rec(number=lit(0))
    entries = loc('entries')
    entry = a.call('list-get', entries, lit(0), types=('@DataEntry',))
    present = a.call('less', lit(0), a.call('list-length', entries, types=('@DataEntry',)))
    value = a.call('data-decode-or', a.nominal_field(entry, 'DataEntry', 'value'), fallback, types=('@Counter',))
    expectation = choose(present, a.variant('DataExpectation', 'Exact', a.nominal_field(entry, 'DataEntry', 'revision')), a.variant('DataExpectation', 'Missing'))
    body = let([('entries', a.get(lit('counter'))), ('old', choose(present, value, fallback)), ('next', rec(number=a.call('add', field(loc('old'), 'number'), lit(1))))], choose(a.put(lit('counter'), loc('next'), '@Counter', expectation), a.json_response(loc('next'), '@Counter'), a.error(409, 'stale')))
    guarded = a.transaction(choose(a.call('$schema-ready'), body, a.error(500, 'schema_mismatch')))
    a.fn('counter', {'request': '@Request'}, '@Response', guarded, ('data',))
    a.port('counter', 'counter', method='POST', path='/api/counter')
    a.fn('health', {'request': '@Request'}, '@Response', a.response(200, a.call('bytes-from-text', lit('{"ok":true,"runtime":"lkjscript"}'))))
    a.port('health', 'health', method='GET', path='/health')
    a.test('integer-arithmetic', a.call('add', lit(40), lit(2)), lit(42))
...
def main():
    ...
    work = Path(tempfile.mkdtemp(prefix='author-', dir=build_root))
    ...
    author.bootstrap()
    if (ROOT / 'tools/game.py').exists():
        from game import compose
        compose(author)
    else:
        prototype(author)
    request = work / 'game.lkjc'
    request.write_text(f'request base={revision}\n' + '\n'.join(author.records) + '\n')
    planned = run('change', 'plan', '--input-file', str(request), '--output', str(work / 'review.logical-plan'), project=project)
    ...
    run('build', '--output', str(artifact), project=project)
    dist = ROOT / 'dist'
    ...
    deployment['http']['maximum_request_body_bytes'] = 32768
    deployment['grants'] = []
    ...
    (dist / 'BUILD.json').write_text(json.dumps({'runtime': 'v0.1.38', 'artifact_sha256': hashlib.sha256(artifact.read_bytes()).hexdigest(), 'accepted_revision': re.findall(r'\brev_[a-f0-9]+\b', applied)[-1], 'authoring_request_sha256': hashlib.sha256(request.read_bytes()).hexdigest()}, indent=2) + '\n')
